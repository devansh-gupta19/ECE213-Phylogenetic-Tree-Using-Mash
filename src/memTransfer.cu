...
void GpuAligner::allocateMem(int32_t len, int32_t numKmers, int32_t kmerSize) {
    ...
    CUDA_CHECK(cudaMalloc(&d_seqs, (len+1) * sizeof(uint32_t)));
    // Zero out the memory so the padded integer is safely 0
    CUDA_CHECK(cudaMemset(d_seqs, 0, (len + 1) * sizeof(uint32_t)));
...
void GpuAligner::transferData2Device(uint32_t *seq, int32_t len, int32_t kmerSize) {
    ...
    //CUDA_CHECK(cudaMemset(d_seqLen, len, sizeof(int32_t)));
    //CUDA_CHECK(cudaMemset(d_kmerSize, kmerSize, sizeof(int32_t)));
}
